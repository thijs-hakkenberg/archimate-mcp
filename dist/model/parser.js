...
import { XmlTypeToElementType, XmlTypeToRelationshipType, } from './types.js';
...
function parseDiagramObjects(children) {
...
function parseElement(elem) {
    ...
    if (!elementType)
        return null; // Not a model element (could be a diagram)
    return {
        id: elem['@_id'] || '',
        type: elementType,
        name: elem['@_name'] || '',
...
function parseRelationship(elem) {
    const xsiType = elem['@_xsi:type'];
    if (!xsiType)
        return null;
    ...
    if (relType === 'Access' && elem['@_accessType']) {
        ...
        rel.accessType = accessMap[elem['@_accessType']];
    }
    // Handle influence modifier
    if (relType === 'Influence' && elem['@_modifier']) {
        rel.influenceModifier = elem['@_modifier'];
    }
    return rel;
}
function parseDiagram(elem) {
    const xsiType = elem['@_xsi:type'];
    if (xsiType !== 'archimate:ArchimateDiagramModel')
        return null;
    return {
        id: elem['@_id'] || '',
        name: elem['@_name'] || '',
...
function parseFolder(xmlFolder) {
    const folder = {
        id: xmlFolder['@_id'],
...
export async function parseModel(modelPath) {
...
function extractRelationships(folder, relationships) {
...
function extractDiagrams(xmlFolder, diagrams) {
    for (const elem of toArray(xmlFolder.element)) {
 ...
 */
export async function parseModelComplete(modelPath) {
    let filePath = modelPath;
    if (!filePath.endsWith('.archimate')) {
        filePath = join(modelPath, MODEL_FILENAME);
    }
    ...
    function processFolder(xmlFolder, parentFolder) {
        const folder = {
            id: xmlFolder['@_id'],
            name: xmlFolder['@_name'],
            type: xmlFolder['@_type'] || '',
 ...
 */
export function getElementById(model, id) {
 ...
 */
export function findElementsByName(model, pattern) {
 ...
 */
export function getRelationshipsForElement(model, elementId, direction = 'both') {
    return model.relationships.filter(r => {
        if (direction === 'outgoing')
            return r.sourceId === elementId;
        if (direction === 'incoming')
            return r.targetId === elementId;
 ...
 * Get folder by type
 */
export function getFolderByType(model, type) {
