...
import { ElementTypeToXmlType, RelationshipTypeToXmlType, } from './types.js';
...
function buildDiagramObject(diagObj) {
    const obj = {
...
function buildFolder(folder, isRelations = false) {
    const obj = {
        '@_name': folder.name,
        '@_id': folder.id,
    };
...
export async function writeModel(model, outputPath) {
 ...
 */
export function createEmptyModel(name, id) {
    return {
        id,
        name,
        version: '5.0.0',
 ...
 */
export function addElementToModel(model, element) {
 ...
 */
export function addRelationshipToModel(model, relationship) {
 ...
 */
export function addDiagramToModel(model, diagram) {
 ...
 */
export function removeElementFromModel(model, elementId) {
    // Remove from folders
    function removeFromFolder(folders) {
        for (const folder of folders) {
    ...
    model.relationships = model.relationships.filter(r => r.sourceId !== elementId && r.targetId !== elementId);
 ...
 */
export function removeRelationshipFromModel(model, relationshipId) {
    ...
    for (const diagram of model.diagrams) {
        for (const obj of diagram.objects) {
            if (obj.sourceConnections) {
                obj.sourceConnections = obj.sourceConnections.filter(c => c.relationshipId !== relationshipId);
            }
        }
    }
    return model;
 ...
 */
export function updateElementInModel(model, elementId, updates) {
    function updateInFolder(folders) {
        for (const folder of folders) {
            ...
            if (element) {
                ...
                if (updates.documentation !== undefined)
                    element.documentation = updates.documentation;
...
function getFolderTypeForElement(elementType) {
    const folderMap = {
        // Motivation
