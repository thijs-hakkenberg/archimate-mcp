...
import { parseModelComplete, getAllElements, getElementById, findElementsByName, getRelationshipsForElement, } from './model/parser.js';
import { writeModel, createEmptyModel, generateId, addElementToModel, addRelationshipToModel, addDiagramToModel, removeElementFromModel, removeRelationshipFromModel, updateElementInModel, } from './model/writer.js';
import { MotivationElementTypes, StrategyElementTypes, BusinessElementTypes, ApplicationElementTypes, TechnologyElementTypes, ImplementationElementTypes, CompositeElementTypes, RelationshipTypes, ElementDescriptions, RelationshipDescriptions, getLayerForElementType, } from './model/types.js';
import { getValidRelationshipTypes, validateRelationship, getRelationshipGuidance, } from './relationships/validation.js';
// =============================================================================
// Server State
// =============================================================================
...
async function handleToolCall(name, args) {
    switch (name) {
        ...
        case 'archimate_save_model': {
            ...
            const savePath = args.path || currentModelPath;
            if (!savePath) {
        ...
        case 'archimate_get_element': {
            ...
            const element = getElementById(currentModel, args.element_id);
            if (!element) {
        ...
        case 'archimate_find_elements': {
            ...
            let elements = findElementsByName(currentModel, args.pattern);
            if (args.layer) {
                elements = elements.filter(e => getLayerForElementType(e.type) === args.layer);
            }
            return [{
        ...
        case 'archimate_create_relationship': {
            ...
            const sourceElement = getElementById(currentModel, args.source_id);
            const targetElement = getElementById(currentModel, args.target_id);
            if (!sourceElement) {
            ...
            const relType = args.relationship_type;
            ...
            const relationship = {
                ...
                influenceModifier: args.influence_modifier,
            };
        ...
        case 'archimate_list_relationships': {
            ...
            if (args.element_id) {
                ...
                relationships = getRelationshipsForElement(currentModel, args.element_id, direction);
            }
        ...
        case 'archimate_update_element': {
            ...
            if (args.documentation !== undefined)
                updates.documentation = args.documentation;
        ...
        case 'archimate_delete_element': {
            ...
            const removedRels = getRelationshipsForElement(currentModel, args.element_id);
            removeElementFromModel(currentModel, args.element_id);
            return [{
                    type: 'text',
                    text: JSON.stringify({
        ...
        case 'archimate_delete_relationship': {
            ...
            removeRelationshipFromModel(currentModel, args.relationship_id);
            return [{
                    type: 'text',
                    text: JSON.stringify({
        ...
        case 'archimate://spec/relationships':
            return {
                contents: [{
                        uri,
                        mimeType: 'application/json',
                        text: JSON.stringify(RelationshipTypes.map(t => ({
                            type: t,
                            description: RelationshipDescriptions[t],
                        })), null, 2),
                    }],
