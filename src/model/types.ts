...
export const AccessTypes = ['Read', 'Write', 'ReadWrite'] as const;
export type AccessType = typeof AccessTypes[number];

export const InfluenceModifiers = ['++', '+', '0', '-', '--'] as const;
...
export function getLayerForElementType(elementType: ElementType): Layer {
  if (MotivationElementTypes.includes(elementType as MotivationElementType)) {
    return 'Motivation';
  }
  if (StrategyElementTypes.includes(elementType as StrategyElementType)) {
    return 'Strategy';
  }
  if (BusinessElementTypes.includes(elementType as BusinessElementType)) {
    return 'Business';
  }
  if (ApplicationElementTypes.includes(elementType as ApplicationElementType)) {
    return 'Application';
  }
  if (TechnologyElementTypes.includes(elementType as TechnologyElementType)) {
    // Check if it's a physical element
    const physicalTypes: ElementType[] = ['Equipment', 'Facility', 'DistributionNetwork', 'Material'];
    if (physicalTypes.includes(elementType)) {
      return 'Physical';
    }
    return 'Technology';
  }
  if (ImplementationElementTypes.includes(elementType as ImplementationElementType)) {
    return 'Implementation';
  }
  if (CompositeElementTypes.includes(elementType as CompositeElementType)) {
    return 'Composite';
  }
...
export const XmlTypeToElementType: Record<string, ElementType> = {
  // Motivation
...
export const ElementTypeToXmlType: Record<ElementType, string> = Object.fromEntries(
  Object.entries(XmlTypeToElementType).map(([xml, elem]) => [elem, xml])
) as Record<ElementType, string>;

export const XmlTypeToRelationshipType: Record<string, RelationshipType> = {
...
export const RelationshipTypeToXmlType: Record<RelationshipType, string> = Object.fromEntries(
  Object.entries(XmlTypeToRelationshipType).map(([xml, rel]) => [rel, xml])
...
export const ElementDescriptions: Record<ElementType, string> = {
  // Motivation
